...
// Determine the recommended workout based on current TSB (Form), active focus and training phase
export function getRecommendedWorkoutType(
  ...
  // interpretTSB()'s "Overtraining risk" zone (shared/pmc.ts, tsb < -25) — this used
  // to trip at -20, a threshold the week-plan view (useTrainingState.ts) didn't share,
  // so a TSB around -22 could show moderate/hard days on the week grid while this
  // single-workout recommender was already forcing recovery.
  if (tsb < -25) {
    return 'recovery';
  }

  // 2. Continuous focus overrides
  ...
  // 3. Event training phase overrides
  ...
  // 4. Standard physiological TSB fallback
  if (tsb < -5) {
    return 'endurance';
  } else if (tsb > 15) {
    return 'threshold';
...
// Generate the workout blocks based on type and available time
export function generateWorkout(
  ...
  const zoneColors = [
    '#94a3b8', // Zone 1 (Slate)
    '#00b894', // Zone 2 (Green)
    '#fdcb6e', // Zone 3 (Tempo/Gold)
    '#ff7675', // Zone 4 (Threshold/Red)
  ...
  switch (type) {
    case 'recovery':
      ...
      blocks.push({ name: 'Recovery Zone', duration: recMain, powerPct: 0.52, zone: 1, color: zoneColors[0] });
      blocks.push({ name: 'Cool-down', duration: recCd, powerPct: 0.40, zone: 1, color: zoneColors[0] });
      break;

    case 'endurance':
      title = 'Aerobic Base Endurance';
      ...
      blocks.push({ name: 'Endurance Zone (Z2)', duration: endMain, powerPct: 0.65, zone: 2, color: zoneColors[1] });
    ...
    case 'sweetspot':
      ...
      // Determine number of intervals based on total time
      // 45m: 2x 10 min (5m recovery)
      // 60m: 2x 15 min (5m recovery)
      // 90m: 3x 15 min (5m recovery)
      let numIntervals = 2;
      ...
      for (let i = 0; i < numIntervals; i++) {
        ...
        if (i < numIntervals - 1) {
          blocks.push({ name: 'Active Recovery', duration: ssRecoveryTime, powerPct: 0.50, zone: 1, color: zoneColors[0] });
        }
      }
    ...
    case 'vo2max': {
      ...
      // 4 min on / 4 min recovery (4x4 Norwegian style) at >= 60 min
      // 40s on / 20s recovery for shorter duration
